...
# This example demonstrates ConsoleColor methods.
...
sub main {
  ...
  Console->WriteLine("All the foreground colors except %s, the background color:",
    ConsoleColor->get($currentBackground));
  foreach my $color (@colors) {
    next if $color == $currentBackground;

    Console->ForegroundColor($color);
    Console->WriteLine("   The foreground color is %s.", ConsoleColor->get($color));
  }
  Console->WriteLine();
  ...
  Console->WriteLine("All the background colors except %s, the foreground color:",
    ConsoleColor->get($currentForeground));
  foreach my $color (@colors) {
    next if $color == $currentForeground;

    Console->BackgroundColor($color);
    Console->WriteLine("   The background color is %s.", ConsoleColor->get($color));
  }
